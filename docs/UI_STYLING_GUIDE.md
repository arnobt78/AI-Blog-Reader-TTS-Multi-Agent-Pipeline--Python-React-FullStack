...
## Blog-to-audio frontend (`frontend/src/index.css`)

Global utility classes (violet-leaning outer glow, aligned with § Shadows above):

| Class | Use |
|-------|-----|
| `glow-card` + `glow-card-hover` | Main glass cards (Reader tool shell, Intro stack / CTA panels) |
| `glow-panel` + `glow-panel-hover` | Tab panels, nested glass surfaces |
| `glow-tabs-shell` | Tab list container behind Paste / Web triggers |
| `glow-dropdown` | Radix `Select` content (see `components/ui/select.tsx`) |
| `glow-field` | Inputs / textareas / select triggers via `fieldSurface` (default violet outer glow; stronger on hover and `:focus-visible`) |
| `glow-card-sm` + `glow-card-sm-hover` | Compact cards (provider chips, intro badges) |

Reader tab labels: **Paste or type** (`text` value) and **From the web** (`url` value); inactive triggers use border + violet wash + shadow on hover.
